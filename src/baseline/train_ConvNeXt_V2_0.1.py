...
def get_config():
    """
    返回训练配置参数 - 针对 ConvNeXt V2 Base (89M) + RTX 4090 24GB GPU 深度优化
    """
    config = {
        # 路径配置
        'base_dir': r'/root/EdgeDeepfakeDetection',
        'log_dir': r'/root/EdgeDeepfakeDetection/experiment/baseline/convnextv2',
        'model_save_dir': r'/root/EdgeDeepfakeDetection/models/convnextv2/seed42_convnextv2_base_4090',
        
        # 随机种子配置
        'random_seed': 42,  # 设为None则不固定种子，使用随机结果；设为整数则确保结果可重现
        
        # RTX 4090 24GB + ConvNeXt V2 Base (89M) 深度优化训练参数
        'batch_size': 96,   # 4090的高效Tensor Core可承载更大batch，但显存相对较少
        'num_workers': 8,   # 4090通常搭配更强CPU，增加数据加载线程
        'num_epochs': 50,   # ConvNeXt 收敛稳定
        'patience': 12,     # 适当增加早停耐心，4090训练速度更快
        'learning_rate': 4e-4,   # 适当提高学习率以匹配更大的batch size
        'weight_decay': 0.05,    # ConvNeXt 常用较大权重衰减
        
        # RTX 4090 优化策略（Ada Lovelace架构）
        'gradient_accumulation_steps': 1,  # 4090性能强，直接使用大batch size
        'mixed_precision': True,   # 开启混合精度，4090 Tensor Core加速更明显
        'pin_memory': True,        # 加速数据传输到GPU
        'prefetch_factor': 8,      # 4090处理速度快，增加预取因子到8
        'persistent_workers': True, # 保持worker进程，减少重启开销
        
        # 运行时优化参数
        'compile_model': True,     # 开启PyTorch 2.0编译优化，4090对新特性支持更好
        'channels_last': True,     # 卷积网络 channels_last 更友好
        
        # 数据参数
        'add_train_noise': False,  # 初次训练不添加噪声，专注模型性能
        'add_test_noise': True,    # 测试时评估噪声鲁棒性
        
        # 学习率调度
        'warmup_epochs': 4,        # 4090训练速度快，减少warmup轮数
        'cosine_eta_min': 1e-6,    # 降低最小学习率，更精细的优化
        'cosine_restart': False,   # 不使用重启，保持稳定收敛
        
        # 正则化
        'dropout_rate': 0.0,            # ConvNeXt 通常不显式使用 Dropout
        'stochastic_depth_rate': 0.2,   # DropPath（Stochastic Depth）
        'label_smoothing': 0.1,    # 增加标签平滑到0.1，提升泛化能力
        
        # RTX 4090 24GB 内存和计算优化
        'amp_dtype': 'bfloat16',   # 4090支持BF16，数值稳定性更好（自动回退到float16）
        'gradient_clip_norm': 1.0, # 梯度裁剪，防止大模型梯度爆炸
        'ema_decay': 0.9999,       # 指数移动平均，稳定训练（可选）
        'use_ema': False,          # 暂不使用EMA，简化训练流程
        
        # 模型特定优化
        'layer_decay': 0.75,       # 保留参数，默认关闭
        'use_layer_decay': False,  # ConvNeXt 默认不开层级衰减
        
        # 保存和检查点优化
        'save_freq': 5,            # 每5个epoch保存一次
        'eval_freq': 1,            # 每个epoch都进行验证
        'log_freq': 100,           # 每100步记录一次日志
    }
    return config
...
def get_mixed_precision_dtype(amp_dtype_str):
    """
    安全地获取混合精度数据类型，如果不支持bfloat16则回退到float16
    """
    try:
        dtype = getattr(torch, amp_dtype_str)
        # 测试是否支持该数据类型
        if amp_dtype_str == 'bfloat16' and not torch.cuda.is_bf16_supported():
            logger.warning("GPU不支持bfloat16，回退到float16")
            return torch.float16
        return dtype
    except AttributeError:
        logger.warning(f"不支持数据类型 {amp_dtype_str}，回退到float16")
        return torch.float16

# ConvNeXt V2 优化的数据增强
def get_data_transforms(target_size=224):
    """
    针对 ConvNeXt V2 Base 优化的数据变换（默认 224）
    """
    return {
        'train': transforms.Compose([
            ...
            transforms.RandomApply([transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 0.5))], p=0.15),
            transforms.ToTensor(),
            # 使用ImageNet预训练的标准化参数
...
# 训练函数（RTX 4090 24GB + ConvNeXt V2 深度优化版本）
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, scaler=None, 
                ...
                gradient_clip_norm=0, channels_last=False, config=None):
    if save_dir is None:
    ...
    # 定义模型保存路径
    best_model_path = os.path.join(save_dir, 'convnextv2_base_best.pth')
    
    # ConvNeXt V2 端到端训练
    logger.info("ConvNeXt V2 Base 使用端到端训练，无需冻结层")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            if scaler is not None:
                dtype = get_mixed_precision_dtype(config.get('amp_dtype', 'float16')) if config else torch.float16
...
if __name__ == '__main__':
    ...
    logger.info("="*80)
    logger.info("ConvNeXt V2 Base Deepfake Detection Training")
    ...
    faces_dir = os.path.join(base_dir, 'data', 'preprocessed', 'FFpp', 'faces_224')
    ...
    # 记录完整的深度优化配置参数
    logger.info("RTX 4090 + ConvNeXt V2 深度优化配置:")
    ...
    # RTX 4090 24GB + ConvNeXt V2 Base 优化的数据加载器配置
    train_loader = DataLoader(
        ...
        pin_memory=config['pin_memory'],           # RTX 4090优化：加速GPU传输
        prefetch_factor=config['prefetch_factor'], # RTX 4090优化：预取数据
        persistent_workers=config['persistent_workers'], # RTX 4090优化：保持worker进程
    ...
    logger.info(f"RTX 4090 + ConvNeXt V2 优化数据加载器配置:")
    logger.info(f"  - batch_size={config['batch_size']} (89M模型在RTX 4090 24GB上的推荐批次大小)")
    ...
    logger.info(f"  - RTX 4090 24GB显存优化，批次大小已调整")
    ...
    logger.info("正在初始化 ConvNeXt V2 Base 模型...")

    model = None

    # 检查本地预训练模型路径（可选）
    local_pretrained_path = os.path.join(config['base_dir'], 'pretrained_models', 'convnextv2_base', 'convnextv2_base.pth')

    if os.path.exists(local_pretrained_path):
        logger.info("检测到本地预训练模型，正在加载...")
        logger.info(f"本地模型路径: {local_pretrained_path}")
        ...
        model = timm.create_model(
            ...
            drop_path_rate=config.get('stochastic_depth_rate', 0.2)
        )

        local_state_dict = torch.load(local_pretrained_path, map_location='cpu')
        if 'model' in local_state_dict:
            local_state_dict = local_state_dict['model']
        elif 'state_dict' in local_state_dict:
            local_state_dict = local_state_dict['state_dict']

        filtered_state_dict = {k: v for k, v in local_state_dict.items() if not k.startswith('head')}
        ...
        logger.info("✓ 成功加载本地预训练模型 (ConvNeXt V2 Base)")
        logger.info(f"  - 跳过的键 (分类头): {[k for k in missing_keys if 'head' in k] if 'missing_keys' in locals() else []}")
    else:
        logger.warning(f"未找到本地预训练模型: {local_pretrained_path}")
        logger.info("尝试使用 timm 预训练权重进行初始化...")
        try:
            model = timm.create_model(
            ...
            logger.info("✓ 已使用 timm 预训练权重 (in22k→in1k) 初始化")
        except Exception as e:
            logger.warning(f"加载 in22k→in1k 权重失败: {e}")
            try:
                model = timm.create_model(
                    'convnextv2_base',
                    pretrained=True,
                ...
                logger.info("✓ 已使用 timm 默认预训练权重初始化")
            except Exception as e2:
                logger.warning(f"加载默认预训练权重失败: {e2}")
                model = timm.create_model(
                    'convnextv2_base',
                    pretrained=False,
                    num_classes=2,
                ...
                logger.info("已使用随机初始化（无预训练）")

    logger.info("模型详细信息:")
    logger.info(f"  - 架构: ConvNeXt V2 (Base)")
    logger.info(f"  - 输入分辨率: 224x224")
    logger.info(f"  - 参数量: ~89M (Base版本)")
    ...
    # ConvNeXt V2 使用 224x224 输入尺寸
    target_size = 224
    logger.info(f"使用 ConvNeXt V2 默认输入尺寸: {target_size}x{target_size}")
    ...
    # RTX 4090 + ConvNeXt V2 训练优化
    model = model.to(device)
    
    # 启用channels_last内存格式（RTX 4090优化）
    ...
    # 层级学习率衰减（ConvNeXt 默认关闭；如需启用，这里演示 stage 级别分组）
    if config.get('use_layer_decay', False):
        ...
        for name, param in model.named_parameters():
            lr_scale = 1.0
            if 'stages.0' in name:
                lr_scale = layer_decay ** 3
            elif 'stages.1' in name:
                lr_scale = layer_decay ** 2
            elif 'stages.2' in name:
                lr_scale = layer_decay ** 1
            elif 'stages.3' in name:
                lr_scale = layer_decay ** 0
            elif 'stem' in name:
                lr_scale = layer_decay ** 4
            elif 'head' in name:
                lr_scale = 1.0  # 分类头使用完整学习率
        ...
        logger.info(f"✓ 使用层级学习率衰减 (decay={layer_decay})")
    else:
    ...
    # 混合精度训练配置（RTX 4090 Tensor Core优化）
    scaler = None
    if config['mixed_precision']:
        scaler = torch.cuda.amp.GradScaler()
        logger.info(f"✓ 混合精度训练已启用 (dtype={config.get('amp_dtype', 'float16')})")
    
    logger.info("ConvNeXt V2 Base 训练配置:")
    logger.info(f"  - 优化器: AdamW {'+ 层级学习率衰减' if config.get('use_layer_decay', False) else ''}")
    ...
    with torch.no_grad():
        for batch_idx, (inputs, labels) in enumerate(tqdm(test_loader, desc="测试进度")):
            ...
            if scaler is not None:
                dtype = get_mixed_precision_dtype(config.get('amp_dtype', 'float16'))
                with torch.amp.autocast('cuda', dtype=dtype):
                    if config.get('channels_last', False):
                        inputs = inputs.to(memory_format=torch.channels_last)
                    outputs = model(inputs)
    ...
    # 训练配置回顾
    logger.info("训练配置回顾:")
    logger.info(f"  模型: ConvNeXt V2 Base (89M参数)")
    logger.info(f"  硬件: RTX 4090 24GB")
    ...
    if torch.cuda.is_available():
        ...
        logger.info(f"  - 内存利用率: {(peak_memory/24)*100:.1f}% (RTX 4090 24GB)")
