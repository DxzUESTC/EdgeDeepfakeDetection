...
def get_config():
    ...
    config = {
        ...
        'model_save_dir': r'D:\09_Project\EdgeDeepfakeDetection\models\baseline\swin_transformer\seed42_swinv2_base',
        
        # 随机种子配置
        'random_seed': 42,  # 设为None则不固定种子，使用随机结果；设为整数则确保结果可重现
        
        # 训练参数
        'batch_size': 16,  # Swin-V2 Base 参数更多，进一步减小批次大小
        'num_workers': 4,
        'num_epochs': 75,  # 增加到75轮，给模型更多时间优化
        'patience': 12,  # 相应增加patience，平衡过拟合与收敛
        'learning_rate': 5e-5,  # 更小的学习率，适应V2 Base的更大参数量（88M）
...
# 数据增强 (调整为256尺寸以适配SwinV2 Small)
data_transforms = {
    'train': transforms.Compose([
        transforms.ToPILImage(),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomResizedCrop(256, scale=(0.8, 1.0)),  # 调整为256以匹配V2模型
    ...
    'val': transforms.Compose([
        transforms.ToPILImage(),
        transforms.Resize((256, 256)),  # 调整为256以匹配V2模型
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
}
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, patience=5, save_dir=None):
    ...
    # 定义模型保存路径
    best_model_path = os.path.join(save_dir, 'swinv2_base_best.pth')
    
    # Swin Transformer V2 不需要冻结特定层，使用较小的学习率即可
    logger.info("Swin Transformer V2 Base 使用端到端训练，无需冻结层")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = os.path.join(save_dir, f'swinv2_base_epoch_{epoch+1}.pth')
...
if __name__ == '__main__':
    ...
    logger.info("="*80)
    logger.info("Swin Transformer V2 Base Deepfake Detection Training")
    ...
    faces_dir = os.path.join(base_dir, 'data', 'preprocessed', 'faces_256')
    ...
    # 加载 Swin Transformer V2 Base 模型
    logger.info("正在初始化Swin Transformer V2 Base模型...")
    try:
        ...
        logger.info("已加载预训练的Swin Transformer V2 Base模型")
        logger.info("模型结构: swinv2_base_window16_256 + Dropout(0.1)")
        logger.info("模型详细信息:")
        logger.info(f"  - 架构: Swin Transformer V2 (Base)")
        logger.info(f"  - 补丁大小: 4x4")
        logger.info(f"  - 窗口大小: 16x16 (相比V1的7x7窗口更大)")
        logger.info(f"  - 输入分辨率: 256x256")
        logger.info(f"  - 嵌入维度: 128")
        logger.info(f"  - 层数: [2,2,18,2] (4个阶段)")
        logger.info(f"  - 注意力头数: [4,8,16,32]")
        logger.info(f"  - 参数量: ~88M (Base版本)")
        logger.info(f"  - V2改进: 相对位置编码、残差后归一化、缩放余弦注意力")
        ...
        logger.info(f"  - V2相对V1改进: 相对位置编码优化、残差后归一化、缩放余弦注意力机制")
        ...
        logger.info("已成功加载Swin Transformer V2 Base模型 + Dropout(0.1)")
        logger.info("模型详细信息:")
        logger.info(f"  - 架构: Swin Transformer V2 (Base)")
        logger.info(f"  - 补丁大小: 4x4")
        logger.info(f"  - 窗口大小: 16x16")
        logger.info(f"  - 输入分辨率: 256x256")
        logger.info(f"  - 嵌入维度: 128")
        logger.info(f"  - 层数: [2,2,18,2]")
        logger.info(f"  - 注意力头数: [4,8,16,32]")
    ...
    logger.info(f"  - 初始学习率: {config['learning_rate']} (针对V2 Base调低)")
    ...
    logger.info(f"  - 批次大小调整: {config['batch_size']} (为V2 Base减小以适应GPU内存)")
    ...
    logger.info(f"  - 噪声鲁棒性 (AUC): {((test_auc_noisy/test_auc)*100):.1f}%")
    logger.info(f"  - 噪声鲁棒性 (AP): {((test_ap_noisy/test_ap)*100):.1f}%")
    logger.info(f"  - 噪声鲁棒性 (EER): {((test_eer/test_eer_noisy)*100):.1f}% (越高越好)")
    logger.info("="*80)